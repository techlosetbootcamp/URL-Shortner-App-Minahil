...
export const PATCH = async (req: NextRequest) => {
    try {
      ...
      if(newUrlCode){
        ...
        if(url){
            return NextResponse.json({ message: "This short url already exists" }, { status: 400 });
        }
        ...
        return NextResponse.json({ message: "urls updated",url,updatedUrl }, { status: 200 });
      }
      else{
...
export const DELETE = async (req: NextRequest) => {
  try {
    ...
    return NextResponse.json({ message: "URL and its analytics deleted successfully" }, { status: 200 });
  } catch (error) {
    return NextResponse.json(
      { message: "Internal Server Error" },
      { status: 500 }
